...
namespace BlazingDartsForFriends.Classes
{
    public class IndexedDbAccessor : IAsyncDisposable
    {
        ...
        public async Task SetValueAtPositionAsync<T>(string collectionName, int position, T value)
        {
            await WaitForReference();
            await _accessorJsRef.Value.InvokeVoidAsync("setAt", collectionName, value, position);
        }
